...
index2(uint64_t page_no = 0, bool refresh_page = false, string view = "overview")
{
    ...
    context["network_info"] = mstch::map {
            ...
            {"difficulty"        , make_difficulty(current_network_info.difficulty,
                                                    current_network_info.difficulty_top64).str()},
            {"cumulative_difficulty", make_difficulty(current_network_info.cumulative_difficulty,
                                                       current_network_info.cumulative_difficulty_top64).str()},
            {"hash_rate"         , hash_rate},
            {"target"            , current_network_info.target},
            ...
            {"tx_count"          , current_network_info.tx_count},
            {"tx_pool_size"      , current_network_info.tx_pool_size},
            {"incoming_connections", current_network_info.incoming_connections_count},
            {"outgoing_connections", current_network_info.outgoing_connections_count},
            {"total_connections" , total_connections},
            {"white_peerlist_size", current_network_info.white_peerlist_size},
            {"grey_peerlist_size" , current_network_info.grey_peerlist_size},
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Qwertycoin address not provided!");
    }
    ...
    // parse string representing given Qwertycoin address
    ...
    if (!xmreg::parse_str_address(xmr_address_str,  address_info, nettype))
    {
        ...
        return string("Cant parse Qwertycoin address: " + xmr_address_str);
    }
    ...
    {
        cerr << "Cant parse the private key (value redacted)" << endl;
        return string("Cant parse private key");
    }
...
//        }
    ...
    if (!generate_key_derivation(pub_key,
                                 ...
                                 derivation))
    {
        cerr << "Cant derive transaction key (private key redacted)" << endl;
    ...
    for (size_t i = 0; i < txd.additional_pks.size(); ++i)
    {
        ...
        {
            cerr << "Cant derive additional transaction key (private key redacted)" << endl;
    ...
    for (output_tuple_with_tag& outp: txd.output_pub_keys)
    {
        ...
        // if someone had sent us some QWC.
    ...
    if (enable_mixin_guess) {
        ...
        for (const txin_to_key& in_key: input_key_imgs)
        {
            ...
            for (const uint64_t& abs_offset: absolute_offsets)
            {
                ...
                {
                    cerr << "Cant derive mixin key (private key redacted)" << endl;
                ...
                for (size_t i = 0; i < mixin_additional_tx_pub_keys.size(); ++i)
                {
                    if (!generate_key_derivation(mixin_additional_tx_pub_keys[i],
                                                 ...
                                                 additional_derivations[i]))
                    {
                        cerr << "Cant derive additional mixin key (private key redacted)" << endl;
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    {
        string error_msg = "Cant parse the private key";
...
search(string search_text)
{
    ...
    // check if Qwertycoin address is given based on its length
    ...
    // check if integrated Qwertycoin address is given based on its length
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Qwertycoin address not provided";
        return j_response;
    }
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Qwertycoin address: " + address_str;
        return j_response;
