...
rpccalls::get_current_height()
{
    ...
    if (!r)
    {
        cerr << "Error connecting to Qwertycoin daemon at "
             << daemon_url << endl;
        return 0;
    }
...
rpccalls::get_network_info(COMMAND_RPC_GET_INFO::response& response)
{
    ...
    if (r)
    {
        ...
        if (!err.empty())
        {
            cerr << "Error connecting to Qwertycoin daemon due to "
                 << err << endl;
            return false;
        }
    }
...
bool
rpccalls::get_epose_info(COMMAND_RPC_GET_EPOSE_INFO::response& response)
{
    epee::json_rpc::request<cryptonote::COMMAND_RPC_GET_EPOSE_INFO::request>
            req_t = AUTO_VAL_INIT(req_t);
    epee::json_rpc::response<cryptonote::COMMAND_RPC_GET_EPOSE_INFO::response, std::string>
            resp_t = AUTO_VAL_INIT(resp_t);
    ...
    {
        cerr << "Error getting EPoSE info from Qwertycoin daemon at "
             << daemon_url << ": " << resp_t.result.status << endl;
        return false;
    }
...
bool
rpccalls::get_service_nodes(COMMAND_RPC_GET_SERVICE_NODES::response& response, uint64_t limit)
{
    COMMAND_RPC_GET_SERVICE_NODES::request req;
    ...
    if (!r || res.status != CORE_RPC_STATUS_OK)
    {
        cerr << "Error getting EPoSE service nodes from Qwertycoin daemon at "
...
rpccalls::get_service_rewards(COMMAND_RPC_GET_SERVICE_REWARDS::response& response, uint64_t height)
{
    ...
    COMMAND_RPC_GET_SERVICE_REWARDS::response res;
    ...
    if (!r || res.status != CORE_RPC_STATUS_OK)
    {
        cerr << "Error getting EPoSE service rewards from Qwertycoin daemon at "
             << daemon_url << ": " << res.status << endl;
        return false;
    }
...
rpccalls::get_block(string const& blk_hash, block& blk, string& error_msg)
{
    ...
    {
        cerr << "get_block: error connecting to Qwertycoin daemon at "
             << daemon_url << endl;
        return false;
    }
