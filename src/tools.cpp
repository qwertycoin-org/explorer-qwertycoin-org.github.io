...
parse_str_secret_key(const string& key_str, T& secret_key)
{
    ...
    {
        cerr << "Cant parse a key (value redacted)" << endl;
        return false;
    }
...
/**
* Parse Qwertycoin address in a string form into
...
/**
* Return string representation of Qwertycoin address
*/
string
...
{
    // default path to the Qwertycoin data folder
    // on linux this is /home/<username>/.qwertycoin
    string default_qwertycoin_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_qwertycoin_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_qwertycoin_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_qwertycoin_dir + string("/lmdb");
}
               ...
               const public_key& public_spend_key)
{
    ...
    {
        cerr << "Cant derive output key (private view key redacted)" << endl;
